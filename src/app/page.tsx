...
export default async function Page() {
  ...
  return (
    <div className="space-y-8">
      {/* Dobra Superior: KPIs Customizados */}
      {activeKpis.length > 0 && (
        <section className={`grid grid-cols-1 md:grid-cols-${Math.min(activeKpis.length, 4)} gap-4`}>
          {kpis.error ? (
            <div className={`col-span-${Math.min(activeKpis.length, 4)} p-4 bg-red-900/20 border border-red-500/50 rounded-lg text-red-400`}>
